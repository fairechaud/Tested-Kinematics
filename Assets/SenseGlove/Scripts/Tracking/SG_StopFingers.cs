...
namespace SG
{
	...
	public class SG_StopFingers : MonoBehaviour
	{
		...
		public SGCore.HandPose Pose;
		public Vector3[][] handAngles;
		public Quaternion[][] handRotations;
		Vector3[][] handPositions;
		//float[] normalFlex = null;
		

		UdpClient clientHands = new UdpClient();//create a client
		...
		float map (float value, float lowIn, float highIn, float lowOut, float highOut)
		{
					...
					// {
						if (String.Equals(handAnimation.Hardware.ToString(), handSideRL)) //Finds which hand is at use: 1.5.1 for left or 1.3.1 for right
						{
							...
							// }		
									...
									if(flexions[0]<0.5)
									{	
										abductionL = Convert.ToInt16(flexions[0]*180); //<------ modify for better finger tracking
										thumbFlexL = Convert.ToInt16(flexions[0]*90);  //<-------- modify for better finder tracking
										// cmdLeft="<1.5.1."+abductionL+"."+thumbFlexL+"."+(int)map(flexions[1],0,1,0,180)+"."+(int)map(flexions[2],0,1,0,180)+"."+(int)map(flexions[3],0,1,0,180)+"."+(int)map(flexions[4],0,1,0,180)+">";
									}else{	
										abductionL = Convert.ToInt16(flexions[0]*180);
										thumbFlexL = Convert.ToInt16(flexions[0]*180);
									}
									// cmdLeft="<1.5.1."+abductionL+"."+thumbFlexL+"."+Convert.ToInt16(map(flexions[1],0,1,0,180))+"."+Convert.ToInt16(map(flexions[2],0,1,0,180))+"."+Convert.ToInt16(map(flexions[3],0,1,0,180))+"."+Convert.ToInt16(map(flexions[4],0,1,0,180))+">";
									// //Debug.Log("<1.5.1."+abductionL+"."+thumbFlexL+"."+Convert.ToInt16(map(flexions[1],0,1,0,180))+"."+Convert.ToInt16(map(flexions[2],0,1,0,180))+"."+Convert.ToInt16(map(flexions[3],0,1,0,180))+"."+Convert.ToInt16(map(flexions[4],0,1,0,180))+">");

									// if (cmdLeft!=null)
									// {
									// 	// send the chest message
									// 	byte[] leftMessage = Encoding.ASCII.GetBytes(cmdLeft);
									// 	clientHands.Send(leftMessage,leftMessage.Length);
									// 	//Debug.Log("Message "+cmdLeft+" sent with success");
									// }
						}else{
							//SG_HandPose(handAngles,handRotations,handPositions,normalFlex);
							...
							// }	
									...
									if(flexions[0]<0.5)
									{	
										abductionR = Convert.ToInt16(flexions[0]*180);//<------ modify for better finger tracking
										thumbFlexR = Convert.ToInt16(flexions[0]*90); //<-------- modify for better finder tracking
									}else{	
										abductionR = Convert.ToInt16(flexions[0]*180); //<------ modify for better finger tracking
										thumbFlexR = Convert.ToInt16(flexions[0]*180); //<-------- modify for better finder tracking
										// cmdRight="<1.3.1."+abductionR+"."+thumbFlexR+"."+flexions[2]*180+"."+flexions[3]*180+"."+flexions[4]*180+">";
									}
									cmdRight=("<1.3.1."+abductionR+"."+thumbFlexR+"."+Convert.ToInt16(map(flexions[1],0,1,0,180))+"."+Convert.ToInt16(map(flexions[2],0,1,0,180))+"."+Convert.ToInt16(map(flexions[3],0,1,0,180))+"."+Convert.ToInt16(map(flexions[4],0,1,0,180))+">");
									//Debug.Log("<1.3.1."+abductionR+"."+thumbFlexR+"."+Convert.ToInt16(map(flexions[1],0,1,0,180))+"."+Convert.ToInt16(map(flexions[2],0,1,0,180))+"."+Convert.ToInt16(map(flexions[3],0,1,0,180))+"."+Convert.ToInt16(map(flexions[4],0,1,0,180))+">");
									if (cmdRight!=null)
									{
//										los son:"+handAngles[0][0]+""+handAngles[0][2]+""+handAngles[1][2]+""+handAngles[2][2]+""+handAngles[3][2]+""+handAngles[4][2]);
										// send the chest message
										byte[] rightMessage = Encoding.ASCII.GetBytes(cmdRight);
										clientHands.Send(rightMessage,rightMessage.Length);
		...
		private void Start()
        {
			...
			clientHands.Connect(new IPEndPoint(IPAddress.Parse("192.168.100.130"),9002));
        }
